// UI 重做 Phase 6：DetailEntertainment —— t-violet 主调 + 共用 Detail 原语
...
export default function DetailEntertainment({ roomId: roomIdProp, embedded = false, onClose }) {
  ...
  const computeJoinState = (r, me) => {
    ...
    if (me && me.status === 'pending')
      return { joinLabel: '等待发起人同意', joinDisabled: true };
  ...
  const join = () => {
    ...
    showModal({
      title: '申请加入',
      content: r.join_rule === 'audit' ? '发起人同意后自动加入' : '点击确认立即加入',
    }).then((res) => {
  ...
  const goEdit = () =>
    nav.navigateTo({ url: `/pages/form-entertainment/form-entertainment?edit=${roomId}` });
  ...
  return (
    <div className={embedded ? 'relative pb-28' : 'relative min-h-screen pb-32 md:pb-12'}>
      {!embedded && <NavBar title="娱乐详情" onBack={handleBack} />}
      <div className={embedded ? 'relative mx-auto w-full px-4 pt-4 md:px-5' : 'relative mx-auto w-full max-w-[860px] px-4 pt-4 md:px-8 md:pt-6'}>
        <DetailHero
          tint="t-violet"
          icon={Sparkles}
          tag={room.ent_type || 'ENTERTAINMENT'}
          title={room.title}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5 mb-4">
          <DetailSection title="活动信息">
            <InfoRow icon={Sparkles} label="类型" value={room.ent_type} />
            <InfoRow icon={Clock3} label="时间" value={room.meet_label} />
            <InfoRow icon={MapPin} label="地点" value={room.meet_location} />
            <InfoRow
              icon={DollarSign}
              label="人均"
              value={room.cost != null ? `¥${room.cost}` : null}
            />
            {room.content && (
              <InfoRow icon={Sparkles} label="补充" value={room.content} />
            )}
          </DetailSection>

          <DetailSection title="发起人">
            <CreatorCard creator={creator} roleLabel="发起人" />
          </DetailSection>
        </div>

        {requirements.length > 0 && (
          <DetailSection title="活动标签" className="mb-4">
